...
public class NumberUtils {
    // 用于匹配手机号码
    private final static String REGEX_MOBILEPHONE = "^0?1[345789]\\d{9}$";
     ...
     * @return
     */
    public static String getAreaCodeFromPhone(String strNumber) {
    ...
    public static Numbers checkNumber(String _number) {
        ...
        if (number != null && number.length() > 0) {
            if (isMobilePhone(number)) {
                ...
                // 获取区号
                String areaCode = getAreaCodeFromPhone(number);
                rtNum = new Numbers(PhoneType.FIXEDPHONE, areaCode, _number);
            } else {
